...
class Subscription extends RecurlyData {
  ...
  all(state, callback) {
    ...
    this.fetchAll('Subscription', `${Subscription.ENDPOINT}?${querystring.stringify({
      state
    })}`, (err, results) => {
  ...
  preview(options, callback) {
    if (!this.id) {
      throw (new Error('cannot preview a subscription without an "id"'))
    }

    let href
    if (this.a && this.a.preview) {
      href = this.a.preview.href
    }
    else {
      href = `${Subscription.ENDPOINT}/${this.id}/preview`
    }

    href += `?${querystring.stringify(options)}`
