...
class Plan extends RecurlyData {
  ...
  all(callback) {
    this.fetchAll('Plan', Plan.ENDPOINT, (err, results) => {
      const data = { }
      _.each(results, plan => {
        data[plan.plan_code] = plan
      })
  ...
  fetchAddOns(callback) {
    ...
    const url = Plan.ENDPOINT + '/' + this.id + '/add_ons';

    this.fetchAll('Addon', url, function(err, results) {
      const addons = { }
      _.each(results, function(addon) {
        addons[addon.add_on_code] = addon
      })
      this._addons = addons;
