...
class Account extends RecurlyData {
  ...
  create(data, callback) {
    ...
    this.post(url, body, (err, response, payload) => {
      ...
      if (response.statusCode === 201) {
        ...
        log.info('[Account/create] End', { debugData, payload })
  ...
  update(callback) {
    ...
    log.info(`[Account/update] Start`, { debugData })

    this.put(this.href, body, (err, response, payload) => {
      const error = handleRecurlyError(err, response, payload, [ 200 ])
      if (error) {
        log.error(`[Account/update] Error`, { err, error, debugData, payload })
        return callback(error)
      }

      this.inflate(payload)
      log.info(`[Account/update] End`, { debugData, payload })
      callback(null, this)
    })
  }
