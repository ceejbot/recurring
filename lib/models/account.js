...
class Account extends RecurlyData {
  ...
  all(state, callback) {
    ...
    this.fetchAll('Account', `${Account.ENDPOINT}?${querystring.stringify({
      state
    })}`, (err, results) => {
      const data = { }
      _.each(results, account => {
        data[account.account_code] = account
      })
      callback(data)
    })
  }
  ...
  update(callback) {
    ...
    const body = data2xml(Account.SINGULAR, new_data)
