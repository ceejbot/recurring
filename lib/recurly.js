...
var
	_           = require('lodash'),
	data2xml    = require('data2xml')({undefined: 'empty', null: 'closed'}),
	path        = require('path'),
	...
	util        = require('util'),
	debugR      = require('debug')('recurring:request'),
...
function Recurring()
{
	...
	{
		CACHE = !!val;
		debug('Cache enabled: %s', CACHE);
	}

	function clearCache()
	{
		delete Account.__all;
		delete Coupon.__all;
		delete Invoice.__all;
		delete Plan.__all;
		delete Subscription.__all;
		delete Transaction.__all;
	}
	...
	{
		debug('Creating RecurlyError from struct: %o', struct);
	...
	function handleRecurlyError(err, response, payload, validStatuses)
	{
		...
		if (recurlyError) {
			...
			console.log('recurlyError in handleRecurlyError', recurlyError);
			return recurlyError;
		}
	...
	function execute(options, callback)
	{
		...
		{
			debugR('execute got response with status code %s: %j', _.get(response, 'statusCode'), body);
			if (err)
			{
	...
	Account.all = function(state, callback)
	{
		...
		if (CACHE && Account.__all && Account.__all[state])
		{
			debug('Using cache');
		...
		RecurlyData.fetchAll(Account, Account.ENDPOINT, function(err, results)
		{
			...
			if (CACHE)
			{
				_.set(Account, '__all[' + state + ']', cache);
			}
			callback(cache);
		});
	};
	...
	var Invoice; // forward reference

	Account.prototype.fetchInvoices = function(callback)
	{
		var self = this;
		if (self.invoices && Array.isArray(self.invoices))
			return callback(null, self.invoices);

		var uri = self._resources.invoices || (self.href + '/invoices');

		RecurlyData.fetchAll(Invoice, uri, function(err, results)
		{
			if (err)
				return callback(err);
	...
	Coupon.all = function(callback)
	{
		...
		RecurlyData.fetchAll(Coupon, Coupon.ENDPOINT, function(err, results)
		{
			...
			{
				cache[coupon.coupon_code] = coupon;
			});
			if (CACHE)
			{
				_.set(Coupon, '__all', cache);
			}
			callback(cache);
		});
	};
	...
	Invoice.all = function(callback)
	{
		...
		RecurlyData.fetchAll(Invoice, Invoice.ENDPOINT, function(err, results)
		{
			...
			{
				cache[invoice.uuid] = invoice;
			});
			if (CACHE)
			{
				_.set(Invoice, '__all', cache);
			}
			callback(cache);
		});
	};
	...
	Invoice.prototype.refund = function(options, callback)
	{
		var self = this;
		...
		var uri = _.get(this, 'a.refund.href');
		...
		var body = data2xml(Invoice.singular, options);
	...
	Plan.all = function(callback)
	{
		...
		RecurlyData.fetchAll(Plan, Plan.ENDPOINT, function(err, results)
		{
			...
			{
				cache[plan.plan_code] = plan;
			});
			if (CACHE)
			{
				_.set(Plan, '__all', cache);
			}
			callback(cache);
		});
	};
	...
	Subscription.all = function(state, callback)
	{
		...
		if (CACHE && Subscription.__all && Subscription.__all[state])
		{
			debug('Using cache');
		...
		RecurlyData.fetchAll(Subscription, Subscription.ENDPOINT + '?' + querystring.stringify({state: state}), function(err, results)
		{
			...
			if (CACHE)
			{
				_.set(Subscription, '__all[' + state + ']', cache);
			}
			callback(cache);
		});
	};
	...
	Subscription.preview = function(options, callback)
	{
		if (!options.plan_code)
			throw(new Error('subscription must include "plan_code" parameter'));
		if (!options.account)
		...
		var body = data2xml(Subscription.singular, options);
		RecurlyData.post(Subscription.ENDPOINT + '/preview', body, function(err, response, payload)
		{
			var error = handleRecurlyError(err, response, payload, [200, 201]);
			if (error)
				return callback(error);
	...
	Transaction.all = function(callback)
	{
		...
		RecurlyData.fetchAll(Transaction, Transaction.ENDPOINT, function(err, results)
		{
			...
			{
				cache[transaction.uuid] = transaction;
			});
			if (CACHE)
			{
				_.set(Transaction, '__all', cache);
			}
			callback(cache);
		});
	};
	...
	Recurring.prototype.setCache = setCache;
	Recurring.prototype.clearCache = clearCache;
