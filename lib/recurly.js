...
RecurlyData.go = function(method, uri, queryargs, opts, callback)
{
...
RecurlyData.post = function(uri, postargs, opts, callback)
{
	if (!callback && typeof opts === 'function') {
		callback = opts;
		opts = {};
	}
	var options = { body: postargs };
	extend(options, opts);
	RecurlyData.go('POST', uri, {}, options, callback);
};
...
RecurlyData.fetchAll = function(Model, uri, callback)
{
	...
	var continuer = function(err, response, records)
	{
		...
		uri = response.headers.link.split(';')[0].slice(1, -1);
