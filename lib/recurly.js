...
RecurlyData.get = function(uri, queryargs, callback)
{
	...
	request(options, function(err, response, body)
	{
		if (err)
		{
			console.error('recurly.get', 'request.get() error ' + JSON.stringify(err));
			return callback(err);
		}
...
RecurlyData.put = function(uri, postargs, callback)
{
	...
	request.put(options, function(err, response, body)
	{
		if (err)
		{
			console.error('recurly.put', 'request.put() error ' + JSON.stringify(err));
			return callback(err);
		}
		if (response.statusCode !== 200)
			return callback(new Error('statusCode: ' + response.statusCode));
		...
		parser.parseXML(body, function(err, result)
		{
			if (err)
			{
				console.error('recurly.put', 'xml parsing error ' + JSON.stringify(err));
				return callback(err, response.headers, {});
			}
...
RecurlyData.delete = function(uri, postargs, callback)
{
	callback('unimplemented');
};
...
Subscription.prototype.update = function(options, callback)
{
	...
	RecurlyData.put(self.href, body, function(err, respHeaders, payload)
	{
		self.inflate(payload);
		console.log(self);
