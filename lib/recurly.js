...
function execute(options, callback)
{
	request(options, function(err, response, body)
	{
		...
		parser.parseXML(body, function(err, result)
		{
			...
			callback(null, response.headers, result);
		});
	});
};
...
RecurlyData.prototype.serialize = function()
{
	// TOOD wrong
	return data2xml(this.properties);
};
...
RecurlyData.prototype.fetch = function(callback)
{
	...
	RecurlyData.get(self.href, {}, function(err, headers, payload)
	{
		if (err)
			return callback(err);
...
Account.prototype.fetchBillingInfo = function(callback)
{
	...
	binfo.href = self.billing_info.href;
...
Account.prototype.create = function(callback)
{
	...
	if (!this.id)
		throw(new Error('you must supply an ID for new accounts'));
	...
	options.first_name = this.first_name;
	options.last_name = this.last_name;
	options.company_name = this.company_name;
	options.accept_language = this.accept_language;

	// TODO optional BillingInfo object

	var self = this;
	var body = data2xml(Account.singular, options);

	RecurlyData.post(Account.ENDPOINT, body, function(err, headers, payload)
	{
		if (err)
...
Account.prototype.destroy = function(callback)
{
	var self = this;
...
var BillingInfo = RecurlyData.buildPrototype({
	properties: [ 'account', 'first_name', 'last_name', 'company',
...
Subscription.prototype.update = function(options, callback)
{
	...
	RecurlyData.put(self.href, body, function(err, headers, payload)
	{
		self.inflate(payload);
		callback(null, self);
	});
...
{
	return data2xml(Subscription.singular, this.properties)
};
...
FormResponseToken.prototype.process = function(callback)
{
	...
	RecurlyData.get(uri, {}, function(err, headers, payload)
	{
		if (err)
			callback(err);
