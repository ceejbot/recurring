...
RecurlyData.fetchAll = function(Model, uri, callback)
{
	...
	var continuer = function(err, response, records)
	{
        ...
        // the response contains links to numerous pages, i.e.,
        // the "start", the "prev", and the "next" page, so we
        // need to make sure we get the correct one... the "next" one.
		var rlinks = response.headers.link.split(',');
        for (var i = 0; i < rlinks.length; i++)
        {
            if (rlinks[i].split(';')[1] == ' rel="next"')
                uri = rlinks[i].split(';')[0].slice(1, -1);
            if (uri.charAt(0) == "<")
                uri = uri.slice(1);
        };
...
Redemption.prototype.__defineGetter__('account_code', function() { return this.account_code; });
